...
leftDownAdjacent x = [x,x-1+rowSize,x-2+2*rowSize,x-3+3*rowSize]
...
res = maximum $ L.map (product . L.map index2Value) (horizontal ++ vertical ++ diagonally)
